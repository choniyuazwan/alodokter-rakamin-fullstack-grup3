class ScheduleDoctorsRepresenter
  ...
  def as_json
    ...
    doctors.each_with_index do |v_doc, i_doc|
      schedules.push({name: v_doc[:name]})

      v_doc.doc_spes.each_with_index { |v_spe, i_spe|
        specialization = schedules[i_doc][:specialization]
        ...
        schedules[i_doc][:specialization] = specialization
      }
    end
    schedules
  end
