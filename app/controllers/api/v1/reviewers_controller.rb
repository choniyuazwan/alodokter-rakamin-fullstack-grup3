  ...
  module V1
    class ReviewersController < ApplicationController
      ...
      def index
        ...
        render json: CommonRepresenter.new(data: RegularsRepresenter.new(@reviewers).as_json, meta: [@reviewers.current_page, @reviewers.limit_value, @reviewers.total_pages, @reviewers.total_count]).as_json
      ...
      def create
        ...
        if @reviewer.save 
          render json: CommonRepresenter.new(data: RegularRepresenter.new(@reviewer).as_json, code: 201).as_json, status: :created
        else
      ...
      def show
        render json: CommonRepresenter.new(data: RegularRepresenter.new(@reviewer).as_json).as_json
      end
